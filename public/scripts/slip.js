    ...
    Slip.prototype = {
        ...
        states: {
            ...
            swipe: function swipeStateInit() {
                ...
                return {
                    leaveState: function() {
                        if (this.holdSwipe && !this.awaitingHoldDecision) {
                                this.awaitingHoldDecision = true
                                this.animateToHold(undefined, this.target)
                        } else if (swipeSuccess) {
                            this.awaitingHoldDecision = false
                            ...
                            this.awaitingHoldDecision = false
                            this.target.node.classList.remove("held")
                            this.animateToZero(removeClass, null, true);
                        }
                    },

                    onMove: function() {
                        var move = this.getTotalMovement();
                        if (this.awaitingHoldDecision) {
                            if (!this.target.node.classList.contains("held")) {
                                let node = window.document.getElementsByClassName("held")[0]
                                if (node) {
                                    this.animateToZero(removeClass, {node}, true)
                    ...
                    onEnd: function() {
                        ...
                        if (!swiped && move.x > 100 && !this.awaitingHoldDecision) {
                            this.target.node.classList.add("held")
                            this.dispatch(this.target.node, 'holdswipe');
                            this.holdSwipe = true
                        } else {
                            this.holdSwipe = false
                        }
        ...
        animateToZero: function(callback, target, zeroOverride) {
            ...
            target.node.style[transformJSPropertyName] = 'translate(0,0) ' + hwLayerMagicStyle + (zeroOverride ? '' : target.baseTransform.value);
            setTimeout(function(){
                target.node.style[transitionJSPropertyName] = '';
                target.node.style[transformJSPropertyName] = zeroOverride ? 'translate(0,0)' : target.baseTransform.original;
                if (callback) callback.call(this, target);
            }.bind(this), 101);
        },

        animateToHold: function(callback, target) {
            // save, because this.target/container could change during animation
            target = target || this.target;

            target.node.style[transitionJSPropertyName] = transformCSSPropertyName + ' 0.1s ease-out';
            target.node.style[transformJSPropertyName] = 'translate(-120px,0) ' + hwLayerMagicStyle + target.baseTransform.value;
            // setTimeout(function(){
            //     target.node.style[transitionJSPropertyName] = '';
            //     target.node.style[transformJSPropertyName] = target.baseTransform.original;
            //     if (callback) callback.call(this, target);
